...
import com.archie.sso.authorize.server.mapper.UserMapper;
...
import com.archie.sso.authorize.server.service.UserRoleService;
    ...
    public UserDetails loadUserByUsername(String username) {
        ...
        try {
            ...
            List<UserRoleEntity> userRoleList = userRoleService.list(
                    Wrappers.lambdaQuery(UserRoleEntity.class).eq(UserRoleEntity::getUserId, user.getUserId()));
            List<String> roleIds = Optional.ofNullable(userRoleList).orElse(Collections.emptyList()).stream()
                    .map(UserRoleEntity::getRoleId).collect(Collectors.toList());
            if (roleIds.isEmpty()) {
                return user;
            }
            // 角色不为空需要返回用户的权限
            List<RoleAuthorityEntity> roleAuthorityList = roleAuthorityService.list(
                    Wrappers.lambdaQuery(RoleAuthorityEntity.class).in(RoleAuthorityEntity::getRoleId, roleIds));
            List<String> authorityIds = Optional.ofNullable(roleAuthorityList).orElse(Collections.emptyList()).stream()
                    .map(RoleAuthorityEntity::getAuthorityId).collect(Collectors.toList());
            if (authorityIds.isEmpty()) {
                return user;
            }
            // 查询权限
            List<AuthorityEntity> authorityList = authorityService.list(
                    Wrappers.lambdaQuery(AuthorityEntity.class).in(AuthorityEntity::getId, authorityIds));
